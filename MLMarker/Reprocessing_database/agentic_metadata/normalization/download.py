...
ONTOLOGY_SOURCES: Dict[str, Dict[str, str]] = {
    ...
    'experimentalfactor': {
        'url': 'https://github.com/EBISPOT/efo/releases/latest/download/efo.obo',
...
def download_ontology(ontology_id: str, 
    ...
    if output_path.exists() and not force:
        ...
        return True, f"  {desc:<40s} exists ({size_mb:.1f} MB)"
    ...
    try:
        ...
        if actual_size < 10 * 1024:
            output_path.unlink()
            return False, f"  {desc:<40s} FAILED (file too small)"

        size_mb = actual_size / (1024 * 1024)
        return True, f"  {desc:<40s} downloaded ({size_mb:.1f} MB)"

    except Exception as e:
        if output_path.exists():
            output_path.unlink()
        return False, f"  {desc:<40s} FAILED ({e})"
...
def create_species_ontology(output_dir: Optional[Path] = None,
    ...
    desc = "Curated Species (NCBITaxon subset)"

    if output_path.exists() and not force:
        size_kb = output_path.stat().st_size / 1024
        return True, f"  {desc:<40s} exists ({size_kb:.1f} KB)"
    ...
    return True, f"  {desc:<40s} created ({len(CURATED_SPECIES)} organisms)"
...
def download_all_ontologies(output_dir: Optional[Path] = None,
    ...
    logger.info("Fetching ontologies for normalization:")

    for ontology_id in ONTOLOGY_SOURCES:
        if skip_large and ontology_id in large_ontologies:
            desc = ONTOLOGY_SOURCES[ontology_id]['description']
            results[ontology_id] = (False, f"  {desc:<40s} skipped (large)")
            logger.info(results[ontology_id][1])
            continue
    ...
    fail = sum(1 for s, _ in results.values() if not s)
    summary = f"Ontologies: {ok} ready"
    if fail:
